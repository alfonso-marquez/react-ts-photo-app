        ...
        <div className="flex items-center justify-center min-h-screen bg-gray-200 w-full p-4">
            <div className="bg-white rounded-lg shadow-md w-full max-w-7xl p-6 md:p-8">
                ...
                <div className="flex flex-col md:flex-row w-full mb-6 justify-between items-start md:items-center gap-4">
                    <h1 className="text-stone-800 font-bold text-xl md:text-2xl">Photos</h1>
                    <AddPhotoDialog setPhotos={setPhotos} fetchPhotos={fetchPhotos} categories={[...new Set(photos.map((photo) => photo.photo_category))]} cameraBrands={[...new Set(photos.map((photo) => photo.camera_brand))]} />
                </div>
                ...
                <div className="flex flex-col md:flex-row w-full gap-4 mb-6">
                    ...
                    <div className="flex flex-wrap items-end justify-end gap-4 flex-1">
                        <SelectFilterPhoto
                            label="Category"
                            options={[...new Set(photos.map((photo) => photo.photo_category))]}
                            ...
                            {filteredPhotos.length > 0 ? (
                                filteredPhotos.map((photo) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-3 py-4"> {format(new Date(photo.photo_taken), "MMM d, yyyy")}</td>

                                        <td className="px-3 py-8 text-right flex gap-2">
                                            <ViewPhotoDialog photo={photo} />
                                            <EditPhotoDialog photo={photo} setPhotos={setPhotos} categories={[...new Set(photos.map((photo) => photo.photo_category))]} cameraBrands={[...new Set(photos.map((photo) => photo.camera_brand))]} />
                                            <DeletePhotoDialog
                                                photo={photo}
                                                setPhotos={setPhotos}
                                            />
